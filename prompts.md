...
This file mirrors `app/prompts.py` for submission. All few-shot router examples from the assignment appear in the Router section.
...
## Exchange Rate Agent (`ExchangeRateAgent`)
...
Dynamic addition at runtime (see `app/agents_setup._general_instructions`): when prior turns exist in `history.json`, a short Hebrew “memory excerpt” block is appended to the system instructions (not shown to the end user as raw JSON).

---

## Guardrail policy (used by code, not as a separate LLM prompt)
...
- **EmptyInputGuardrail** / **SafetyInputGuardrail** — SDK `@input_guardrail` functions on `RouterAgent`, blocking mode (`run_in_parallel=False`), deterministic regex/heuristics (no extra LLM).
- **RouterOutputGuardrail** — deterministic validation: handoff JSON is parsed into Pydantic models; `RouterDecision` is logged; failures are demonstrated offline via `python -m app.main --demo-output-guardrail`.
- **FinalAnswerSafetyGuardrail** / **NonEmptyFinalAnswerGuardrail** — SDK `@output_guardrail` on each specialist agent.
